...
# 2. 데이터 준비 (열처리 열 제외 및 특정 열만 선택)
# 원본 데이터 순서 유지를 위해 Material 열 따로 관리 필요 시
materials = df.iloc[:, 0].values

# X (Input) 설정: 범주형(HT)을 제외한 핵심 변수 선택
# 1~11번 인덱스: C, Si, Mn, P, S, Cr, Mo, B, Ni, Al, Cu (화학 성분)
# 19~21번 인덱스: HB, TS, E (기계적 성질)
X_chemical = df.iloc[:, 1:12].values 
X_mechanical = df.iloc[:, 19:22].values 

# 두 그룹을 가로로 결합 (Total 14개 Features)
X = np.concatenate([X_chemical, X_mechanical], axis=1)

# y (Target): W~Z열 (Index 22~25) -> Fatigue Parameters
y = df.iloc[:, 22:26].values
target_names = ['sigma_f', 'b', 'e_f', 'c']

print(f"입력 특징량(X) 개수: {X.shape[1]}") # 14가 나오는지 확인
...
output_filename = 'KFold_Analysis_김도진_NoHT_31.xlsx'
